...
ny_dt = utc_dt.astimezone(eastern)
print(f"New York Time (with DST if applicable): {ny_dt}")
...
def fetch_forex_data(fx_pair):
    ...
    eastern = pytz.timezone('America/New_York')
    fx_timestamp = datetime.fromtimestamp(res.last.timestamp / 1000.0, eastern)
    if quote_currency == "JPY":
        rate = res.converted/100
    else:
        rate = res.converted
...
from pycaret.regression import setup, compare_models, predict_model, save_model, load_model, pull
...
def create_synth_pair(cycle):
    ...
    for i in range(lim):
        avg_max = (eurusd_data[i]['max_value'] + gbpchf_data[i]['max_value'] + usdcad_data[i]['max_value'] + eurchf_data[i]['max_value']
                   + eurcad_data[i]['max_value'] + gbpeur_data[i]['max_value']  + gbpcad_data[i]['max_value'] + usdchf_data[i]['max_value']) / 8
        avg_min = (eurusd_data[i]['min_value'] + gbpchf_data[i]['min_value'] + usdcad_data[i]['min_value'] + eurchf_data[i]['min_value']
                   + eurcad_data[i]['min_value'] + gbpeur_data[i]['min_value'] + gbpcad_data[i]['min_value'] + usdchf_data[i]['min_value']) / 8
        avg_mean = (eurusd_data[i]['mean_value'] + gbpchf_data[i]['mean_value'] + usdcad_data[i]['mean_value']+ eurchf_data[i]['mean_value']
                    + eurcad_data[i]['mean_value'] + gbpeur_data[i]['mean_value']  + gbpcad_data[i]['mean_value']
                    + usdchf_data[i]['mean_value'] ) / 8
        avg_vol = (eurusd_data[i]['vol_value'] + gbpchf_data[i]['vol_value'] + usdcad_data[i]['vol_value']  + eurchf_data[i]['vol_value']
                   + eurcad_data[i]['vol_value'] + gbpeur_data[i]['vol_value'] + gbpcad_data[i]['vol_value']
                   + usdchf_data[i]['vol_value'] ) / 8
        avg_fd = (eurusd_data[i]['fd_value'] + gbpchf_data[i]['fd_value'] + usdcad_data[i]['fd_value'] + eurchf_data[i]['fd_value']
                  + eurcad_data[i]['fd_value'] + gbpeur_data[i]['fd_value']  + gbpcad_data[i]['fd_value']
                  + usdchf_data[i]['fd_value'] ) / 8
